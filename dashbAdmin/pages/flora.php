<?php
if (isset($_GET['hapus'])) {
    $coll_del = ($_GET['lang'] ?? 'id') === 'en' ? 'flora_en' : 'flora';
    fb_delete($coll_del, $_GET['hapus']);
    header("Location: ?page=flora&msg=hapus&lang=" . ($_GET['lang'] ?? 'id')); exit;
}
...
<div class="content-card">
    <div class="card-header-custom">
        <h6><i class="bi bi-flower2 text-primary me-2"></i>Daftar Flora</h6>
        <a href="?page=form_tambah_flora&lang=<?= $lang ?>" class="btn btn-primary btn-sm btn-action">
    ...
    <div class="table-toolbar">
        ...
        <div class="ms-auto d-flex gap-2">
            <a href="?page=flora&lang=id" class="btn btn-sm <?= $lang==='id'?'btn-success':'btn-outline-success' ?> btn-action">
                <i class="bi bi-translate me-1"></i>ID
            </a>
            <a href="?page=flora&lang=en" class="btn btn-sm <?= $lang==='en'?'btn-success':'btn-outline-success' ?> btn-action">
        ...
        <table class="table table-hover mb-0" id="dataTable">
            ...
            <?php $no = 1; foreach ($data as $row): ?>
                <tr>
                    ...
                    <td>
                        <a href="?page=detail_flora&id=<?= $row['__id'] ?>&lang=<?= $lang ?>"
                           class="btn btn-sm btn-outline-info btn-action me-1">
                            <i class="bi bi-eye"></i>
                        </a>
                        <a href="?page=form_edit_flora&id=<?= $row['__id'] ?>&lang=<?= $lang ?>"
                           class="btn btn-sm btn-outline-warning btn-action me-1">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <a href="?page=flora&hapus=<?= $row['__id'] ?>&lang=<?= $lang ?>"
            ...
            <?php endforeach; ?>
            <?php if (empty($data)): ?>
                <tr><td colspan="6" class="text-center text-muted py-5">Belum ada data flora.</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
