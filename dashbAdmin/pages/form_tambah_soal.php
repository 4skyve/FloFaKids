...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['question'])) {
        $err = 'Pertanyaan wajib diisi.';
    } elseif (fb_insert($coll, $data)) {
        header("Location: ?page=soal&msg=tambah&lang=$lang"); exit;
    } else {
        ...
        <div class="content-card">
            <div class="card-header-custom">
                <h6>
                    ...
                    Tambah Soal — <i class="bi bi-translate me-1"></i><?= strtoupper($lang) ?>
                </h6>
            </div>
                ...
                <form method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="opt<?= $i ?>" class="form-control"
                               value="<?= htmlspecialchars($_POST["opt$i"] ?? '') ?>">
                    </div>
                    <?php endfor; ?>
                    <div class="mb-3">
                        ...
                        <select name="answer" class="form-select" required>
                            <?php for ($i = 0; $i < 4; $i++): ?>
                                <option value="<?= $i ?>" <?= (int)($_POST['answer'] ?? 0)===$i?'selected':'' ?>>
                                    Opsi <?= $i+1 ?>
                                </option>
                            <?php endfor; ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="level" class="form-control" min="1" required
                               value="<?= htmlspecialchars($_POST['level'] ?? '1') ?>"
